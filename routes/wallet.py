...
TRX_PER_TON = 10_000_000
MIN_WITHDRAW = 100

@wallet_bp.route("/api/convert", methods=["POST"])
@auth_required
def api_convert(tg):
    ...
    if direction == "trx_to_ton":
        if user["balance"] < TRX_PER_TON:
            conn.close()
            return jsonify({"error": f"Need {TRX_PER_TON:,} TRX"}), 400
        conn.execute("UPDATE users SET balance=balance-?, ton_balance=ton_balance+1 WHERE user_id=?",
                     (TRX_PER_TON, tg["id"]))
